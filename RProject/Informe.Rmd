...
#### Analisis por generation
En este apartado haremos un seguimiento y comparación de las distintas generaciones. Primero agruparemos los datos por generaciones,
mostraremos el número de suicidios totales y relativos e identificaremos que generación ha sufrido mayor riesgo.

```{r}
#Mostrar generaciones disponibles
#table(dfSuicidiosFiltered$generation)
```

```{r}
#Agrupar por generaciones, con todas las columnas posibles. Media de HDI, media de suicides_100k_pop y media de gdp_per_capita, sexo y año
aggregate(dfSuicidiosFiltered, by = dfSuicidiosFiltered$generation)
```

```{r}
#Grafica de barras de los suicidios totales y los suicidios relativos ordenando las generaciones de vieja a nueva
```
...
**SACAR CONCLUISIONES DE LOS RESULTADOS**
